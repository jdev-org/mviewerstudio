var mv = (function () {
    ...
    return {
        ...
        createDublinCore(data) {
            ...
            if (data?.versions?.commits && data.versions.commits) {
                dataDate = data.versions.commits.filter(f => f.current)[0]?.date;
                dataDate = moment(dataDate, "YYYY-MM-DD-HH-ss-mm")
            }
            var thedate = dataDate.toISOString();
            var themes = [];
            ...
            fetch(`${_conf.api}?search=${search}`)
                ...
                .then(data => {
                            ...
                            <div class="list-group-item">
                                <div class="row">
                                    <div class="col-md-6 liste_applications_info">
                                        ...
                                        <div class="app-subjects">Thématiques&nbsp;: ${ app.subjects || "-" }</div>
                                        <div class="app-group ${ !app.group ? 'd-none' : '' }">Groupe&nbsp;: ${ app.group }</div>
                                        <div class="app-subjects">Identifiant&nbsp;: ${ app.id }</div>
                                        <div class="app-subjects">Mots clés&nbsp;: ${ app.keywords }</div>
                                        <div class=app-creator">Auteur&nbsp;: ${ app.creator }</div>
                    ...
                    if (!applications.length) {
                        ...
                        `
                        document.querySelector("#liste_applications").innerHTML = emptyResult;
                    }
        ...
        createVersion: (id) => {
            ...
            const promises = [
                ...
                fetch(_conf.api, {
                    ...
                    return fetch(`${ _conf.api }/${id}/version`, {method: "POST"})
                    .catch(err => console.log(err))
                    .then(r => r.json())
                })
            ];
