    ...
    return {
        ...
        showLayerOptions: function (el) {
            ...
            if (layer.type === "wms") {
                ...
                ogc.getWfsInfosFromWms(layer.url, layerid).then(
                    ({ describeLayer }) => {
                    ...
                    ogc.getFeatures(wfs_url, { TYPENAME: layerid, MAXFEATURES: 1 }, (data) => {
                        if (config.temp.layers[layerid]) {
                            ...
                            mv.createDispatchEvent("wfsFeaturesReady", {features: config.temp.layers[layerid].features})
                        }
                    })
        ...
        writeLayerNode: function (l) {
            ...
            if (l.templateurl && l.useexternaltemplate) {
                ...
                template = '<template url="' + l.generatorTemplateUrl + '" ></template>';
            }
            if (!template && l.templateFromGenerator) {
        ...
        parseApplication(xml) {
            ...
            if (proxy && proxy.getAttribute) {
                // apply proxy from xml config file
                optProxyUrl.value = proxy?.getAttribute("url");
            } else {
                // apply default proxy from json config
                optProxyUrl.value = _conf.proxy;
            }
            ...
            themes.each(function (id, th) {
                addTheme($(th).attr("name"), ($(th).attr("collapsed") || true), $(th).attr("id"), $(th).attr("icon"), $(th).attr("url"), $(th).attr("layersvisibility"));
                ...
                layers.each(function (id, l) {
                    ...
                    var layer = {
                        ...
                        useexternaltemplate: ($(l).find("template") && $(l).find("template").text().length > 3 || ($(l).find("template").attr("url") && ($(l).find("template").attr("url").length > 1))),
                        templateurl: false,
                ...
                if ($(th).attr("url")) {
                    ...
                    $(".themes-list-item[data-themeid='"+$(th).attr("id")+"']").attr("data-theme-layersvisibility", layersvisibility);
                } else {
        ...
        showListApplications(data, search) {
            ...
            data
                ...
                .forEach((app) => {
                    ...
                    let previewUrl = "";
                    if (app.relation) {
                        let filePath = `${ mv.getAuthentUserInfos("groupFullName") }/${ app.relation }`;
                    ...
                    const navButtons = [{
                        ...
                        html: `
                            <li class="nav-item">
                                <a class="nav-link" href="#" data-id="${ app.id }" data-url="${ app.url }" onclick="window.open('${ previewUrl }','mvs_vizualize')">Prévisualiser</a>
                            </li>`
                    }, {
                        show: app.creator == _userInfo.userName,
                        html: `
                                <li class="nav-item">
                                    <a class="nav-link" href="#" data-url="${ _conf.mviewer_instance + app.link }" data-group="${ app.group }" onclick="loadApplicationParametersFromRemoteFile(this.attributes[\'data-url\'].value)">Modifier</a>
                                </li>`
                    }, {
                        show: !_conf.is_php && app.creator == _userInfo.userName,
                        html: `
                                <li class="nav-item">
                                    <a class="nav-link" href="#" data-id="${ app.id }" data-bs-dismiss="modal" onclick="showAlertDelAppFromList(this.attributes[\'data-id\'].value)">Supprimer</a>
                                </li>`
                    }];
                    
                    let badgeLabel = app.relation ? mviewer.tr("publish") : mviewer.tr("draft");
                    let badgeColor = config.relation && app.relation ? "badge-publish" : "badge-draft";
        ...
        formatPublishText: (value, row) => {
            ...
            let contentText = value.includes("publication") ? mviewer.tr("publish.message") : value;
        ...
        actionVersionFormatter: (value, row, index) => {
            ...
            return `
                <a id="versionPreviewLink" onclick="mv.previewVersion('${ config.id }', '${ row.id }')"><span class="custom-tooltip-table"><i class="ri-eye-line"></i><span>Prévisualiser cette version</span></span></a>
                <a id="versionAsNewLink" data-bs-target="#genericModal" data-bs-toggle="modal" onclick="mv.showAlertChangeVersion('${ config.id }', '${ row.id }', true)"><span class="custom-tooltip-table"><i class="ri-arrow-go-back-fill"></i><span>Restaurer cette version</span></span></a>
                ${ row?.version ? flag : "" }
            `;
        },
        appsTableToolbar: () => {
            ...
            <div class="toolbar" style="display:inline-flex">
                ...
                <div class="custom-control custom-switch mt-2 ml-2" id="btnSwitchHistoryTagsOnly">
                    ...
                    <label class="custom-control-label" for="inputSwitchHistoryTagsOnly"><span>Voir les versions marquées</span></label>
                </div>
            </div>
            `;
        },
        ...
        showPublishModal: (shareLink = "", iframeLink = "", draftLink = "") => {
            ...
            genericModalContent.innerHTML = `
                <div class="modal-header">
                    <h5 class="modal-title" i18n="modal.publish.title">Publiée avec succès</h5>
        ...
        publish: (id, name = "", message = "") => {
            ...
            if (!message) {
                url += `?message=${ message || mviewer.tr("publish.message") }`;
            }
            fetch(
                url,
                {
        ...
        produceUrl: (filepath, isPublish = false, newConf = {}) => {
            ...
            if (isPublish) {
                ...
                return _conf.mviewer_instance + '?config=' + _conf.conf_path_from_mviewer + filepath
            }
        },
        openTemplateGenerator: () => {
            ...
            if (!data) {
                ...
                document.addEventListener("wfsFeaturesReady", () => {
                    mv.templateGenerator = new mv.templateGeneratorComponent(config.temp.layers[layerId].features)
