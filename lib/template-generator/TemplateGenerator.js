...
export default class TemplateGenerator {
  ...
  addToSelection = ({ detail }) => {
    ...
    // init tooltips
    [...document.querySelectorAll(".cardComponent .tooltip-info")].forEach(el => new bootstrap.Tooltip(el))
  };
  /**
  ...
  callTemplate = () => {
    if (this.layer.templateFromGenerator) {
      ...
      let templateFullPath = this.layer.templateurl; 
      if (this.layer.generatorTemplateUrl) {
        templateFullPath = `${ _conf.conf_path_from_mviewer }${ this.layer.generatorTemplateUrl }`;
        templateFullPath = _conf.mviewer_instance + templateFullPath;
      }
      if (!templateFullPath) return;
      fetch(templateFullPath).then(response => response.text()).then(mstContent => {
  ...
  readTemplate = (mstContent) => {
    ...
    this.layer.useGeneratorTemplate = true;
