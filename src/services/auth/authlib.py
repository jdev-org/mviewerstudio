...
def start_authlib_login():
    ...
    return authlib_client().authorize_redirect(
        url_for("auth-routes.authlib_callback", _external=True)
    )
...
def _user_claims_from_token(token: dict[str, object]) -> dict[str, object]:
    ...
    if metadata.get("userinfo_endpoint"):
        ...
        if hasattr(response, "json"):
            return _coerce_claims(response.json())
        return _coerce_claims(response)
    return {}
...
def complete_authlib_login():
    ...
    claims = normalize_claims(_user_claims_from_token(token))
...
def build_authlib_logout_redirect() -> str:
    ...
    redirect_uri = request.host_url.rstrip("/") + app_root_path()
