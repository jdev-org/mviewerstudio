var mv = (function () {
  ...
  return {
    ...
    /**
     * Open the reusable geolocation workflow for a Grist layer.
     *
     * @param {Object} layer Grist layer being edited.
     * @returns {void}
     */
    openGristLayerGeolocation: function (layer) {
      mv.components.grist.openGristLayerGeolocation({
        layer,
        mvInstance: mv,
        appConfig: _conf,
        translator: mviewer,
    ...
    writeGroupNode: function (g) {
      ...
      var group = [padding(8) + "<group "];
      $.each(groupParameters, function (prop, parameter) {
        group.push(padding(12) + parameter);
      });
      group.push(">");
    ...
    showListApplications(data, search) {
      ...
      data
        ...
        .forEach((app) => {
          ...
          let badge = `<span class="badge ${badgeColor}">${badgeLabel}</span>`;
          const items = `
    ...
    showNamePublishModal: (id, name = "", conflict = false) => {
      ...
      genericModalContent.innerHTML = `
              ...
              <div class="modal-body">
                  ...
                  <a id="sendPublishApp" class="cardsClose save-close zoomCard" data-bs-dismiss="modal" onclick="mv.publish('${id}', mv.nameNormalizer(document.getElementById('relationPublishName')?.value)), mv.setNewTheme(false)">
